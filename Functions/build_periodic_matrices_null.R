...
build_periodic_matrices_null <- function(ua, data, hb.site = NULL){
  ...
  for(m in seq_along(ua$phi.a.mu)){
    ...
    theta.32 <- ifelse(gdd <= 1000 | gdd >= rho.a[m], boot::inv.logit(ua$grow.na.mu[m]), 0)
    theta.21 <- ifelse(gdd >= rho.n[m] & gdd <= 2500, boot::inv.logit(ua$grow.ln.mu[m]), 0)  
    ...
    A[3,3,,m] <- boot::inv.logit(ua$phi.a.mu[m] + alpha.a[m])
    A[1,3,,m] <- lambda
  }
  return(A) 
}