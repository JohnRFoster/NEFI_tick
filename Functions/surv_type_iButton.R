...
surv_type_iButton <- function(subset, position){
  ...
  met <- read.csv("../TickClimate_iButtonData.csv", stringsAsFactors = FALSE)
  # wrong.fd <- which(met$Site == "FD ")
  # met$Site[wrong.fd] <- "FD"
  wrong.fd <- which(met$X...Site == "FD ")
  met$X...Site[wrong.fd] <- "FD"
  met <- met %>% 
  ...
  rawdata <- read.csv("../Cary_Larvae_Nymph_Surv.csv")
  dat <- rawdata
