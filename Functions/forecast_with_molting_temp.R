...
forecast_with_molting_temp <- function(type, site, params, ic, N_est, df, threshold, Nmc, draw){
  ...
  for(m in 1:Nmc){
    ...
    for(t in 1:(N_est)){   # loop over the number of sampling days
      ...
      nymph.molt[t,year.col] <- Nprev[1,1] * TRANS[2,1]
      adult.molt[t,year.col] <- Nprev[2,1] * TRANS[3,2]
      larva.molt[t,year.col] <- Nprev[3,1] * TRANS[1,3]
