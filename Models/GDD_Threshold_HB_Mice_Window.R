...
run_model <- function(n.adapt,n.chains){
  ...
  tau.21 ~ dgamma(0.01,0.01)       # random site effect: larvae-to-nymph transition
  ...
  logit(t21[s,t]) <- grow.ln.mu + beta.21*mice[s,t] + alpha.21[s]
  logit(t32[s,t]) <- grow.na.mu + beta.32*mice[s,t] + alpha.32[s]
  
  theta.21[s,t] <- ifelse((gdd[s,t] >= 500) || (gdd[s,t] >= 2000),t21[s,t],0)
