...
run_model <- function(site.run,n.adapt,n.chains,burnin,thin,n.iter){
  ...
  inits <- function(){list(x = data$y,
                           repro.mu = rnorm(1, 4, 0.001),
                           phi.l.mu = rnorm(1, 4.21098, 0.001),
                           phi.n.mu = rnorm(1, 2.8828, 0.001),
                           phi.a.mu = rnorm(1, 5, 0.001),
                           grow.ln.mu = rnorm(1, -4.21098, 0.001),
                           grow.na.mu = rnorm(1, -2.8828, 0.001),
                           alpha.22 = rnorm(3, 0, 0.001),
                           alpha.33 = rnorm(3, 0, 0.001),
                           alpha.13 = rnorm(3, 0, 0.001),
                           alpha.32 = rnorm(3, 0, 0.001),
                           beta.11 = rnorm(1, -0.05187, 0.007537),
                           beta.22 = rnorm(1, 27.2245, 16.1133),
  ...
  ### global hyperpriors
  phi.l.mu ~ dnorm(4.21098,0.2438)               # larvae survival
  phi.n.mu ~ dnorm(2.8828,1.7954)               # nymph survival
  phi.a.mu ~ dnorm(5,1)               # adult survival
  grow.ln.mu ~ dnorm(-4.21098,0.1)             # larvae -> nymph transition 
  grow.na.mu ~ dnorm(-2.8828,0.1)             # nymph -> adult transition
  ...
  ### Fixed effect - Daily Max Temp - priors
  beta.11 ~ dnorm(-0.05187,17603.6599)
  beta.22 ~ dnorm(27.2245,0.003851)
  ...
  logit(A.day[1,1,s,t]) <- phi.l.mu + beta.11*met[t,3,s] 
  logit(A.day[2,1,s,t]) <- grow.ln.mu 
  logit(A.day[2,2,s,t]) <- phi.n.mu + beta.22*met[t,3,s] + alpha.22[s]
  logit(A.day[3,2,s,t]) <- grow.na.mu + alpha.32[s] 
  logit(A.day[3,3,s,t]) <- phi.a.mu + beta.33*met[t,3,s] + alpha.33[s] 
