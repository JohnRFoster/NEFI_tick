predict_state_one_null <- function(type, site, params, ic, data, Nmc, draw){
  ...
  df <- data$df[s,]
  col.alpha.13 <- paste("alpha.13[", s, "]", sep = "")
  col.alpha.22 <- paste("alpha.22[", s, "]", sep = "")
  col.alpha.32 <- paste("alpha.32[", s, "]", sep = "")
  ...
  if("parameter" %in% type){
    ...
    alpha.13 <- params[draw, col.alpha.13]
    alpha.22 <- params[draw, col.alpha.22]
    alpha.32 <- params[draw, col.alpha.32]
  } else {
    ...
    alpha.13 <- rep(param.mean[col.alpha.13], Nmc)
    alpha.22 <- rep(param.mean[col.alpha.22], Nmc)
    alpha.32 <- rep(param.mean[col.alpha.32], Nmc)
  }
  ...
  # select appropriate random effects
  # re-assign alphas based on site-to-site variability
  ...
  for(m in 1:Nmc){
    ...
    A[1,1,] <- inv.logit(phi.l.mu[m])
    A[1,3,] <- exp(repro.mu[m] + alpha.13[m])
    A[2,1,] <- inv.logit(grow.ln.mu[m])
    A[2,2,] <- inv.logit(phi.n.mu[m] + alpha.22[m])
