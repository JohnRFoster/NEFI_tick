...
##   
##   Transition is a window threshold on larvae-to-nymph survival 
##
...
run_model <- function(n.adapt,n.chains,burnin,thin,n.iter){
  ...
  tau.11 ~ dgamma(0.01,0.01)       # random site effect: nymph survival
  tau.22 ~ dgamma(0.01,0.01)       # random site effect: larvae-adult transition
  ...
  for(s in 1:3){
    ...
    ## daily nymph and survival by site
