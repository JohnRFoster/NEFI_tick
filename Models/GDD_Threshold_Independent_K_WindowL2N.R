...
##
##   The threshold for transition is a random variable, driven by cumulative gdd
##
...
run_model <- function(site.run,n.adapt,n.chains,burnin,thin,n.iter){
  ...
  theta.21[t] <- ifelse((gdd[t] >= k.l2n.low) && (gdd[t] <= k.l2n.high),grow.ln.mu,0)
  theta.32[t] <- ifelse((gdd[t] <= k.n2a.low) || (gdd[t] >= k.n2a.high),grow.na.mu,0)
