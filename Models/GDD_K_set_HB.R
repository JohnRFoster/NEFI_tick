...
run_model <- function(met.proc, n.adapt, n.chains,init.dir,init.model.name){
  
  data <- cary_ticks_met_JAGS()
  data <- hb_restructure(data, met.proc)
  data$R <- diag(1, 3, 3)
  ...
  init.params <- get_inits_HB(init.dir, init.model.name, met.proc)
  ...
  inits <- function() {
    list(
      ...
      phi.a.mu = rnorm(3, init.params["phi.a.mu"], 0.01),
  ...
  for(m in mis.temp.min){
    temp.min[m] ~ dunif(temp.min.range[1], temp.min.range[2])
  }
  # for(m in met.mis){
  #   met[m] ~ dunif(met.mis.range[1], met.mis.range[2])
  # }
  ...
  for(t in 1:max(N_days)){   # loop over every day in time series
    ...
    for(s in 1:3){
      ...
      logit(A.day[3,3,t,s]) <- phi.a.mu[s]
  ...
  for(s in 1:3){
    for(t in 1:N_est[s]){
      ...
      theta.larva[t,s] <- 1 / (1 + beta.l.obs*(temp.min[dt.index[s,t]])^2)
